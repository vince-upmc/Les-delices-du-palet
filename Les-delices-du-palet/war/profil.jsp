...
<body>
	...
	<div id="main-content">
		<div class="left-panel">
			...
			<%
				DateFormat profileFormat = new SimpleDateFormat("dd-MM HH:mm",
							Locale.FRANCE);
			...
			<div class="profile-wrapper">
				<div class="bets">
					<table>
						...
						<tr>
							<td class="bet-title" colspan="6">Paris en cours</td>
						</tr>
						<%
							for (Pari p : paris) {
									if (!p.isDone()) {
							...
							<td>
								<%
									switch (p.getBetObjective()) {
												case Home:
								%><a href="/team.jsp?team-id=<%=home.getId()%>"><%=home.getName()%></a>
								<%
									;
													break;
												case Away:
 ...
 %>
						...
						<tr>
							<td class="bet-title" colspan="6">Anciens paris</td>
						</tr>
						<%
							for (Pari p : paris) {
									if (p.isDone()) {
										...
										Team away = pm.getObjectById(Team.class, m.getAway());
						%>
						<tr <%if (p.isBetSuccessful(m)) {
							wins++;%>
							style="color: green" <%} else {%> style="color:red" <%}%>>
							<td>
								<%
									switch (p.getBetObjective()) {
												case Home:
 ...
 %>
							...
							<td><%=m.getBoxScore().getHomeScore() + " - "
								+ m.getBoxScore().getAwayScore()%></td>
							<td><%=p.getMise() + " / "%> <%
